...
public class DrawingDoubly extends SurfaceView implements SurfaceHolder.Callback   {
	float x,y;
	...
	DoublyFunction df = new DoublyFunction();
    ...
    public DrawingDoubly(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    class AnimationThread extends Thread {
        ...
        public void run() {
            ...
            while (mRun) {
                ...
                try {
                	...
                	Rect bg = new Rect();
         			bg.set(0, 0, DrawingDoubly.this.getWidth(), DrawingDoubly.this.getHeight());
         			...
         			p.setTextSize(13*px);
         			bg.set(1,1,DrawingDoubly.this.getWidth()-1,DrawingDoubly.this.getHeight()-1);
         			c.drawRect(bg,p);
         			if(i != 0){
           ...
           for(int i = 0; i < data.length; i++){
        	...
        	if(i>0){
        		if(fix && ipos == i){
    	        	...
    	        	canvas.drawLine((initial - 8)*px, 108*px, (initial - 20)*px, 108*px, p);
    	        	canvas.drawText("<", (initial - 25)*px, 112*px, p);
    	        	initial = initial + x;
        ...
        private void DrawText(Canvas canvas){
           ...
           for(int i = 0; i < data.length; i++){
           	...
           	if(data[i].length() > 2){
           		if(fix && ipos == i){
           			canvas.drawText(data[i].substring(0, 2)+"",(initial + 3)*px,500*px, p);}
           		else{
        		...
        		if(fix && ipos == i){
        			canvas.drawText(data[i]+"",(initial + 3)*px,500*px, p);}
        		else{
            ...
            for(int i = 0; i < data.length; i++){
            	...
            	if(fix && ipos == i){
    	        	...
    	        	canvas.drawRect(r, p);
    	 			...
    	 			p.setStyle(Paint.Style.FILL);
        			canvas.drawLine(((initial)+10)*px, 150*px, ((initial)+10)*px, 90*px, p);
        			canvas.drawText("v", ((initial)-9)*px, 90*px, p);
